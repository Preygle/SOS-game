...
def highlight_cell(x, y):
    begX = gridX + x * (grid_size + space) + space // 2 + line_thickness / 2
    begY = gridY + y * (grid_size + space) + space // 2 + line_thickness / 2

    highlight = shapes.Rectangle(begY, begX,
                                 grid_size - space - line_thickness,
                                 grid_size - space - line_thickness,
                                 color=(255, 255, 0), batch=overlay_batch)
    return highlight
...
def on_mouse_press(x, y, button, modifiers):
    ...
    if gridX <= x <= gridX + cellSize and gridY <= y <= gridY + cellSize:
        ...
        if board[rows][cols] == ' ':
                ...
                hc = highlight_cell(rows, cols)
                
@window.event
def on_key_press(symbol, modifiers):
    global sprites, current_player
    ...
    if selected_cell:
        ...
        if board[rows][cols] == ' ':
            ...
            scored = check_win()  # Store result to determine whether to switch players
